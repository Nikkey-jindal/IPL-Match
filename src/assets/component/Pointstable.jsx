...
let [data,setData]=useState(pointsdata.pointsTable[0].pointsTableInfo);
 
  return (
    <div className='w-full bg-black flex justify-center text-white text-[10px] h-screen'>
         <table className='w-[70%] bg-gray-900 h-screen  ' >
        <tr className='text-center  font-bold mt-4  '>
          <td className='text-left w-[20%] sm:[30%]'>Team</td>
          <td>M</td>
          <td>W</td>
          <td>L</td>
          <td>Pts</td>
     ...
     data.map((item,i)=>{
      return (
      <tr className='text-center  mt-2 border-t border-gray-400/20' 
      >
       ...
       <td>{item.points}</td>
       <td>
  {item.form.reverse().map((value, index) => {
  
    return value === "W" ? (
      <i
        key={index}
        className="fi fi-ss-check-circle text-green-600"
      ></i>
    ) : (
      <i
        key={index}
        className="fi fi-ss-cross-circle text-red-600"
      ></i>
    )
  })}
</td>
